...
import pandas as pd
try:
    from .selection import record_choice
except ImportError:
    from selection import record_choice
...
def apply_dynamic_history(
    ...
    for index in order:
        ...
        if bool(row["is_flexible"]) and bool(row["runtime_prediction_reliable"]):
            ...
            if options:
                best = record_choice(result, index, options, [
                    ("not_a_delay", lambda x: x["release"] > initial_offset),
                    ("capacity_saving_below_threshold", lambda x: x["capacity_pct"] >= min_carbon_saving_pct),
                    ("node_saving_below_threshold", lambda x: x["node_pct"] >= min_node_saving_pct),
                    ("carbon_return_below_threshold", lambda x: x["return"] >= min_carbon_return),
                    ("non_positive_utility", lambda x: x["utility"] > 0),
                ])
                if best is not None:
                    chosen = best
...
def apply_dynamic_forecast(
    ...
    for index in order:
        ...
        if bool(row["is_flexible"]):
            ...
            best = record_choice(result, index, options, [
                ("not_a_delay", lambda x: x["release"] > initial_offset),
                ("no_incremental_wait", lambda x: x["incremental_wait"] > 0),
                ("capacity_saving_below_threshold", lambda x: x["capacity_pct"] >= min_carbon_saving_pct),
                ("node_saving_below_threshold", lambda x: x["node_pct"] >= min_node_saving_pct),
                ("carbon_return_below_threshold", lambda x: x["return"] >= min_carbon_return),
                ("non_positive_utility", lambda x: x["utility"] > 0),
...
def apply_dynamic_marginal(
    ...
    for index in order:
        ...
        if bool(row["is_flexible"]):
            ...
            best = record_choice(result, index, options, [
                ("not_a_delay", lambda x: x["offset"] > initial_offset),
                ("risk_adjusted_saving_below_threshold", lambda x: x["risk_adjusted_pct"] >= min_carbon_saving_pct),
                ("carbon_return_below_threshold", lambda x: x["return"] >= min_carbon_return),
                ("non_positive_utility", lambda x: x["utility"] > 0),
            ], release_key="offset")
            if best is not None:
    ...
    for index in order:
        ...
        if bool(row["is_flexible"]):
            ...
            for candidate in candidate_release_times(carbon, eligible, deadline, runtime_s):
                ...
                options.append({"score": score, "release": candidate_offset,
                                "intensity": candidate_intensity, "mean_load": mean_load,
                                "peak_load": peak_load,
                                "saving_pct": (before-candidate_intensity)/before*100 if before else 0.0})
            best = record_choice(result, index, options, [
                ("capacity_saving_below_threshold", lambda x: x["release"] == initial_offset or x["saving_pct"] >= min_carbon_saving_pct),
            ], score_key="score", maximize=False)
            if best is not None:
                chosen_offset = best["release"]
                after = best["intensity"]
                objective = best["score"]
                mean_load, peak_load = best["mean_load"], best["peak_load"]
            else:
                mean_load, peak_load = projected_class_load(
...
def write_scenario(output: Path, profile: pd.DataFrame, summary: dict[str, object]) -> None:
    ...
    try:
        ...
        from reporting import decision_report
    decisions, candidates = decision_report(profile)
    decisions.to_csv(output / "v2_job_decisions.csv", index=False)
    candidates.to_csv(output / "v2_candidate_audit.csv", index=False)
    summary = {**summary, "implementation": "v2_feasible_first", "frozen_campaign_result": False}
